...
class TradingEngine:
    def __init__(self, update_queue, config, telegram=None):
        ...
        # --- APLICA O MODO TESTE AQUI ---
        if self.config.TEST_MODE:
            self.exchange.set_sandbox_mode(True) # Ativa Testnet
            print("\n⚠️  ATENÇÃO: MODO TESTE (SANDBOX) ATIVADO! ⚠️")
            print("    O bot usará dinheiro fictício da Binance Testnet.")
            # Envia aviso no log
            self.update_queue.put(('log', "⚠️ MODO TESTE ATIVADO: Usando Binance Testnet"))
        # --------------------------------
